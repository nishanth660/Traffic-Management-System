...
class TrafficSimulatorWithSignals:
    def __init__(self, output_path='traffic_simulation_with_signals.mp4', duration=60, resolution=(800, 800)):
        ...
        self.cars = []  # List of car dicts: {'pos': (x, y), 'dir': 'N','color': color,...etc}

        # Traffic lights per lane: dict of {direction: 'red'/'green'}
        ...
        # Car spawn probabilities per lane
        self.spawn_prob = 0.1

        # Fixed lanes for cars to spawn (at edges of frame) - N,S,E,W approach
        # Each lane has a starting point and movement direction vector
        self.lanes = {
            'N': {'start': (self.center_x - self.lane_width // 2, 0), 'dir': (0, 1), 'lane_center_y': self.center_y - self.lane_width//2},
            'S': {'start': (self.center_x + self.lane_width // 2, self.height), 'dir': (0, -1), 'lane_center_y': self.center_y + self.lane_width//2},
            'E': {'start': (self.width, self.center_y - self.lane_width // 2), 'dir': (-1, 0), 'lane_center_x': self.center_x + self.lane_width//2},
            'W': {'start': (0, self.center_y + self.lane_width // 2), 'dir': (1, 0), 'lane_center_x': self.center_x - self.lane_width//2},
        }
    ...
    def create_background(self):
        ...
        # Draw lane markings - dashed for vertical road
        for y in range(0, self.height, 30):
            cv2.line(bg,
                     (self.center_x, y),
                     (self.center_x, y + 15),
                     (255, 255, 255), 2)
        # Draw lane markings - dashed for horizontal road
    ...
    def spawn_cars(self):
        # Spawn new cars randomly in each lane according to spawn probability
        for direction, lane_info in self.lanes.items():
            if random.random() < self.spawn_prob:
                ...
                car = {
                    ...
                    'speed': speed,
                    'stopped': False  # Is car stopped at red light
                }
    ...
    def move_cars(self):
        ...
        for car in self.cars:
            ...
            # Determine stop line position based on lane direction
            stop_line = None
            if dir == 'N':
                stop_line = self.center_y - self.road_width//2 - self.car_length//2
                # Check if red signal in that direction, and if car is near stop line, stop it
            ...
            # Remove car if has passed frame boundary (opposite side)
    ...
    def count_waiting_cars_per_direction(self):
        ...
        for car in self.cars:
            ...
            dir = car['dir']
            if dir == 'N':
                # Cars before stop line waiting at red
    ...
    def update_signals(self):
        # Adaptive signal timing based on waiting car counts on each side

        # Total waiting cars in NS and EW groups
        ...
        # Update signal timer count
        self.current_signal_time += 1

        # Duration to switch signals depends on relative traffic volume
        if self.active_green_group == 'EW':
            green_duration = int(
                np.clip(EW_count * 4 + self.min_green_time, self.min_green_time, self.max_green_time))
            if self.current_signal_time > green_duration * self.fps:
                # Switch to NS green
                self.signals = {'N': 'green', 'S': 'green', 'E': 'red', 'W': 'red'}
                self.active_green_group = 'NS'
                self.current_signal_time = 0
        else:  # NS green active
            green_duration = int(
                np.clip(NS_count * 4 + self.min_green_time, self.min_green_time, self.max_green_time))
            if self.current_signal_time > green_duration * self.fps:
                # Switch to EW green
                self.signals = {'N': 'red', 'S': 'red', 'E': 'green', 'W': 'green'}
    ...
    def draw_countdown(self, frame):
        # Draw countdown timer near center for current green group
        if self.active_green_group == 'EW':
            green_duration = int(
                np.clip(sum([c for d, c in self.count_waiting_cars_per_direction().items() if d in ['E', 'W']]) * 4 + self.min_green_time, self.min_green_time, self.max_green_time))
            ...
            green_duration = int(
                np.clip(sum([c for d, c in self.count_waiting_cars_per_direction().items() if d in ['N', 'S']]) * 4 + self.min_green_time, self.min_green_time, self.max_green_time))
            countdown_sec = max(0, green_duration - self.current_signal_time / self.fps)
    ...
    def run(self):
        ...
        for _ in range(self.total_frames):
            ...
            # Spawn new cars probabilistically
            self.spawn_cars()

            # Update signal lights adaptively
            self.update_signals()

            # Move cars based on signal states
            self.move_cars()
            ...
            # Optional: To view live, uncomment below (press q to quit)
