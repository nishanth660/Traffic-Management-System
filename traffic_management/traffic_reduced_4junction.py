...
class TrafficSimulatorWithSignals:
    def __init__(self, output_path='traffic_simulation_with_signals.mp4', duration=60, resolution=(800, 800)):
        ...
        self.spawn_prob = 0.03  # Lower spawn probability for less cars
    ...
    def create_background(self):
        ...
        cv2.rectangle(bg, (self.center_x - self.road_width // 2, 0),
                      (self.center_x + self.road_width // 2, self.height),
                      (100, 100, 100), -1)
        cv2.rectangle(bg, (0, self.center_y - self.road_width // 2),
    ...
    def move_cars(self):
        ...
        # Check which directions are already occupied in the junction
        for car in self.cars:
            x, y = car['pos']
            dir = car['dir']
            # Junction boundaries
            ...
            if car['in_junction']:
                # This car currently occupies its direction
        ...
        for car in self.cars:
            ...
            in_junction = (jx1 <= new_x <= jx2 and jy1 <= new_y <= jy2)
            ...
            car['pos'] = (x, y)
            if car['in_junction']:
                # After car passes junction zone, mark as not in junction
                if not (jx1 <= x <= jx2 and jy1 <= y <= jy2):
                    car['in_junction'] = False
